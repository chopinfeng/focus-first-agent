...
| E15 | 阶段回看：四种呈现物的认知负载 | bypass 下人靠 transcript 回看，静默决策召回 < 50%；证据包 ≥ 90% 且读得更少 | 真实运行的产物、24–30 人 | 真人，在线 | **P0，可与 E3 合并** |
...
**假设**
- H15a：bypass 模式的成本不为零：同一段执行，读原始 transcript 的时间是读 FFA 证据包的 ≥ 4 倍。
- H15b：静默决策召回率：transcript < 50%，diff < 30%，Agent 自述 ≈ 假设列表的覆盖率，FFA 证据包 + 已替你决定队列 ≥ 90%。
- H15c：不可逆动作召回率：只有 FFA 达到 100%；transcript 下漏掉的多为"白名单命令的副作用"（脚本删文件）。
- H15d：回看阶段 NASA-TLX：transcript > diff > 自述 > FFA。

**材料**：用 demo 服务在 bypass 模式跑 6 个任务（`MODE=bypass`），导出四种呈现物；每段执行有 ground truth：Agent 做了哪些决定、哪些不可逆动作（服务端 `review.silent` 与 `review.irreversible`）。

**设计**：被试内，四种呈现物拉丁方平衡，每人看 4 段不同执行。每段回看后回答：Agent 替你做了哪些决定（自由列举）、发生了哪些不可逆动作、这段能否合并（判断）、TLX。

**规模**：24–30 人。

**指标**：read_time、silent_recall、irreversible_recall、merge 判断正确率、TLX。

**判定**：H15b 不成立（transcript 召回也很高）→ 任务太小，换更长的执行段（≥ 40 次工具调用）；FFA 召回 < 90% → 证据包里"已替你决定"与不可逆标记的呈现需要改。

**成本**：材料由 demo 自动生成，1 人日；招募约 30 × 15 美元。
