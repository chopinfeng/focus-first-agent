...
- `demo/` — **接入真实 LLM 的本地服务**：`npm start` 后打开 http://localhost:8765，三个 Agent 在沙箱里做真实任务，每个工具调用经内核路由后实时渲染到 demo 页面；支持 Anthropic SDK、任意 OpenAI 兼容接口，以及无需 key 的 `LLM_PROVIDER=mock`。见 `demo/README.md`
